...
//
// This is a hardware port of rig-module-firmware.ino (LilyGo T-CAN485) onto
// the Waveshare ESP32-S3-RS485-CAN board. config.h/modbus.h/scaling.h/
// webui.h are UNCHANGED and shared byte-for-byte with the LilyGo variant —
// only this .ino (pins + LED handling) differs. Keep both in sync manually
// when touching shared logic; there's no build system tying them together.
//
...
//     this board has no separate UART-to-USB chip, unlike the LilyGo)
//   Flash Size: 16MB, PSRAM: OPI PSRAM (board is ESP32-S3R8: 16MB flash + 8MB PSRAM)
...
// Verified against Waveshare's own schematic + two independent open-source
// ports (Battery-Emulator issue #2334, esphome-yambms board doc):
//   RS485: TX=GPIO17, RX=GPIO18, DE/RE=GPIO21 (SP3485 transceiver — DE and
//          /RE are tied together: HIGH=transmit, LOW=receive)
//   CAN:   TX=GPIO15, RX=GPIO16 (native ESP32-S3 TWAI + onboard transceiver
//          — not used by this firmware yet, available for future CAN work)
// Unlike the LilyGo T-CAN485, this board's RS485 direction is NOT automatic
// — modbus.h's existing DE-pin toggle logic (already written for the
// LilyGo) is exactly what's needed here too, just pointed at GPIO21.
// There is no 5V-booster-enable pin and no RS485-shutdown pin on this
// board (the isolated RS485 section is always powered/enabled once the
// board itself has power) — so PIN_5V_EN/RS485_SE from the LilyGo variant
// don't exist here and are simply omitted.
// =============================================================================
#define RS485_TXD   17   // Serial2 TX
#define RS485_RXD   18   // Serial2 RX
#define RS485_DE    21   // RS485 DE/RE (driver enable, active HIGH = transmit)

// No programmable WS2812/NeoPixel on this board — only fixed PWR/RS485/CAN
// indicator LEDs wired directly to hardware, not GPIO-controllable. All
// ledSet()/ledTick() calls elsewhere in this file are kept as harmless
// no-ops below so the rest of the sketch (poll task, WiFi state machine,
// etc.) needs zero changes.
static void ws2812Init() { /* no LED on this board */ }
static void ws2812Set(uint8_t r, uint8_t g, uint8_t b) { (void)r; (void)g; (void)b; }
...
void setup() {
  ...
  // No 5V-booster-enable or RS485-shutdown pins on this board (unlike the
  // LilyGo T-CAN485) — the isolated RS485 section is always powered/enabled
  // as soon as the board itself has power, so there's nothing to toggle
  // here before modbusInit().
...
void connectWifi() {
  ...
  if (found == 0) {
    ...
    for (int i = 0; i < found; i++) {
      ...
      // This ESP32-S3 is 2.4GHz-only (same as the LilyGo's plain ESP32) — channels 1-14.
