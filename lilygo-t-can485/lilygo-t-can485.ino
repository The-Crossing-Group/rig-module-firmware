// =============================================================================
// lilygo-t-can485.ino — Generic Rig Module Firmware
// LilyGo T-CAN485 / XY-32 CAN+RS485 (ESP32, WROOM-32) — bench-testing board
// Reads Waveshare Modbus RTU Analog Input 8CH (B) or Eletechsup AMIDJ14 via RS485
...
//
// This is a hardware port of waveshare-s3.ino (Waveshare ESP32-S3-RS485-CAN)
// onto the LilyGo T-CAN485 board, bringing that variant's full feature set
// (v1.11.0: board auto-detect, AMIDJ14 digital I/O, RPM Pulse Counter Mode
// incl. its configurable fast-poll timeout, multi-board /advanced, baud
// auto-detect) to the bench-testing hardware. config.h/modbus.h/scaling.h/
// pulse.h/webui.h are UNCHANGED and shared byte-for-byte with the
// waveshare-s3 variant — only this .ino (pins + WS2812 LED handling)
// differs. Keep both in sync manually when touching shared logic; there's
// no build system tying them together.
//
// This supersedes the older rig-module-firmware.ino (repo root, stuck at
// v1.7.1 — predates board auto-detect, AMIDJ14 digital I/O, and RPM Pulse
// Counter Mode entirely). Use this folder for any new LilyGo work.
//
