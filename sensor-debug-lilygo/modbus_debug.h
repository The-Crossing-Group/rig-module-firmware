...
// tool is meant to be reconfigured every session). Uses whatever pins
// were passed to dbgSerialInit() originally - not hardcoded, so this
// file works unmodified across board variants (Waveshare, LilyGo, ...).
void dbgSerialApply(uint32_t baud, char parity, int stopBits) {
  if (!_dbgSerial) return;
  _dbgSerial->flush();
  _dbgSerial->end();
  delay(20);
  _dbgSerial->begin(baud, dbgSerialConfig(parity, stopBits), _dbgRxPin, _dbgTxPin);
  _dbgCfg.baud = baud;
  _dbgCfg.parity = parity;
  _dbgCfg.stopBits = stopBits;
  delay(20);
